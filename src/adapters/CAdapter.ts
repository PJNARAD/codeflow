  ...
  async execute(prepared: PrepareResult, fuelLimit: number, onStep: (step: UniversalExecutionStep) => void): Promise<ExecutionResult> {
    ...
    for (let i = 0; i < lines.length && fuelConsumed < fuelLimit; i++) {
      ...
      if (line.includes('malloc')) {
        ...
        const addr = 0x2000 + Math.floor(Math.random() * 8000);

        const ptrMatch = line.match(/(\\w+)\\s*=\\s*malloc/);
        if (ptrMatch && step.variables[ptrMatch[1]]) {
          step.variables[ptrMatch[1]] = { ...step.variables[ptrMatch[1]], value: addr, pointsTo: 'heap+' + addr };
        }

        step.heap = { [addr]: { value: 'allocated', type: 'void*', size, allocationSite: { line: i + 1, function: 'main' } } };
      ...
      if (line.includes('free(')) {
        ...
        if (match && step.variables[match[1]]?.address) {
          const addr = step.variables[match[1]].address;
          if (step.heap?.[addr]) step.heap[addr].value = 'freed';
      ...
      if (line.includes('*') && line.match(/\\*\\s*\\w+/)) {
        ...
        if (match && step.variables[match[1]]?.isPointer && step.variables[match[1]]?.pointsTo) {
          step.reason = 'Dereferenced ' + match[1] + ' -> ' + step.variables[match[1]].pointsTo;
          step.fuelUsed += 30;
        }
      }
    ...
    return {
      ...
      output: steps.filter(s => s.output).map(s => s.output),
      errors,
      fuelConsumed
    };
  }
